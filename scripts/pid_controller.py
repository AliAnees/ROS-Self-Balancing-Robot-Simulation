...
pub_right = rospy.Publisher(
    '/teeterbot/right_speed_cmd', Float64, queue_size=10)
...
def controller(data):
    ...
    roll = euler[0]
    pitch = euler[1] #Tilt of the body
    yaw = euler[2]

    error = set_angle*np.pi/180 - pitch #Desired angle - current angle

    print(error*180/np.pi, error_sum)

    #Adding to the error sum, aka area under curve for integral control
    if abs(error_sum) < 3:
    ...
    #Calculating for diff in error, foe derivative control
    ...
    #Calculating target speed of wheels using trapezoid method
    target_speed = kp*error + ki * error_sum + kd * error_diff

    #Communicating desired speeds with the robot
